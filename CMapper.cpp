...
std::string m_sPath = "C:\\Users\\topor\\source\\repos\\TestDll\\Release\\TestDll.dll";
...
void CMapper::ProcessMapping ( ) {
    ...
    m_aMappedImage.resize ( m_aImage.size ( )  );
    std::memcpy ( m_aMappedImage.data ( ), m_aImage.data ( ), m_aImage.size ( ) );
}

std::map< std::string, std::string > CMapper::GetImports ( ) {
    std::map< std::string, std::string > m_aImportList = { };

    for ( auto& m_Import : m_aImports ) {
        std::string m_sCurrentModule = std::string ( m_Import.m_pszModule );

        for(auto&  func : m_Import.m_aFunctions )
            m_aImportList.insert ( std::make_pair ( func, m_sCurrentModule ) );

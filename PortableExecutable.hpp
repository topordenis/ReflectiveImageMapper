...
struct LIMAGE_DATA_DIRECTORY {
    std::uint32_t VirtualAddress;
    std::uint32_t Size;
};

struct LIMAGE_OPTIONAL_HEADER32 {
    std::uint16_t Magic;
...
struct LIMAGE_OPTIONAL_HEADER64 {
    std::uint16_t Magic;
    std::uint8_t MajorLinkerVersion;
    std::uint8_t MinorLinkerVersion;
    std::uint32_t SizeOfCode;
...
struct LIMAGE_NT_HEADERS {
    std::uint32_t Signature;
    LIMAGE_FILE_HEADER FileHeader;
    LIMAGE_OPTIONAL_HEADER32 OptionalHeader;
    LIMAGE_OPTIONAL_HEADER64 OptionalHeader64;
    std::uint16_t OptionalMagic;
};

struct LIMAGE_IMPORT_DESCRIPTOR {
    union {
...
struct LIMAGE_IMPORT_BY_NAME {
    std::uint16_t Hint;
    CHAR Name [ 1 ];
};

struct LIMAGE_BASE_RELOCATION {
    std::uint32_t VirtualAddress;
    std::uint32_t SizeOfBlock;
};

struct LIMAGE_SECTION_HEADER {
...
#define IMAGE_FIRST_SECTION( m_pNTHeader ) ( ( LIMAGE_SECTION_HEADER* )        \
